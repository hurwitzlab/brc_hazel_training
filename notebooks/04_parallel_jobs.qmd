...
- `-J "$JOB1[1-$NUM_JOBS]%$NUM_JOBS"`: Array syntax with optional concurrency limit
...
| [1-\$NUM\_JOBS] | e.g., `[1-50]` | This defines the **range** of the job array. It means the array will consist of sub-jobs with indices starting from **1** up to the value of $NUM\_JOB (which is the number of lines in your input file). Each sub-job will process one line/sample. |
| %\$NUM\_JOBS | e.g., `%50` | This is the crucial **concurrency limit**. It specifies the **maximum number of sub-jobs** from this array that LSF is allowed to run **at the same time** (concurrently). |
